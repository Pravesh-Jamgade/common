...
char * BlockTypeString(CacheBlockInfo::block_type_t block_type) {
   switch(block_type)
   {
      case CacheBlockInfo::block_type_t::PAGE_TABLE:              return "page_table";
      case CacheBlockInfo::block_type_t::NON_PAGE_TABLE:          return "non_page_table";
      case CacheBlockInfo::block_type_t::SECURITY:                return "security";
      case CacheBlockInfo::block_type_t::EXPRESSIVE:              return "expressive";
      case CacheBlockInfo::block_type_t::UTOPIA:                  return "utopia";
      ...
      Core::mem_origin_t mem_origin)
{
   ...
   // if(metadata_request)
   // {
   //       //std::cout << "Mem op from core metadata: address " << ca_address << std::endl;   
   // }
   ...
   if(shadow_cache && metadata_request){
      ...
      // For page table metadata: perform lookup inside the shadow cache 
      shadow_cache_lookup = shadow_cache->lookup(ca_address, t_start, true, count);
      
      if(shadow_cache_lookup == UtopiaCache::where_t::HIT)
         shadow_cache_hit = true;
      //std::cout << "Shadow_cache lookup: " << shadow_cache_hit << std::endl; 
   }
   ...
   if (cache_hit)
   {
      ...
      {
         /*std::cout<<"MSHR\n";
         for (int i = 0; i < m_master->mshr.size(); i++)
         {
            std::cout<<"Mshr Entry:"<<m_master->mshr[i].t_issue<<" "<<m_master->mshr[i].t_complete<<"\n";
         }*/
         if(!metadata_request){
            ScopedLock sl(getLock());
            ...
            // This is a hit, but maybe the prefetcher filled it at a future time stamp. If so, delay.
            SubsecondTime t_now = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
            if (m_master->metadata_mshr.count(ca_address)
               && (m_master->metadata_mshr[ca_address].t_issue < t_now && m_master->metadata_mshr[ca_address].t_complete > t_now))
            {
   ...
   SubsecondTime t_end_final = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
...
CacheCntlr::Prefetch(IntPtr eip,SubsecondTime t_now)
{
   ...
   {
      doPrefetch(eip, address_to_prefetch, m_master->m_prefetch_next);
...
CacheCntlr::doPrefetch(IntPtr eip, IntPtr prefetch_address, SubsecondTime t_start)
{
   ...
   HitWhere::where_t hit_where = processShmemReqFromPrevCache(eip, this, Core::READ, prefetch_address, true, true,CacheBlockInfo::block_type_t::NON_PAGE_TABLE, Prefetch::OWN, t_start, false, Core::mem_origin_t::NORMAL);
   ...
   if (hit_where == HitWhere::MISS)
   {
      ...
      hit_where = processShmemReqFromPrevCache(eip, this, Core::READ, prefetch_address, false, false,CacheBlockInfo::block_type_t::NON_PAGE_TABLE, Prefetch::OWN, t_start, false, Core::mem_origin_t::NORMAL);
   ...
   if (cache_hit)
   {
      ...
      if (modeled)
      {
         if(!metadata_request){
            ...
            // This is a hit, but maybe the prefetcher filled it at a future time stamp. If so, delay.
            SubsecondTime t_now = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
            if (m_master->metadata_mshr.count(address)
               && (m_master->metadata_mshr[address].t_issue < t_now && m_master->metadata_mshr[address].t_complete > t_now))
            {
   ...
   else
   {
      ...
      if (isPrefetch != Prefetch::OWN)
      {
         ...
         if (! cache_hit) {
            ...
            //std::cout << "Increase Reuse for address: " << address << "with reuse: " << getCache()->peekSingleLine(address)->getReuse() <<std::endl;
...
{
   /* Keep only last 8 MSHR entries */
   while(m_master->metadata_mshr.size() > 8) {
      IntPtr address_min = 0;
      SubsecondTime time_min = SubsecondTime::MaxTime();
