...
{
   std::cout << "Allocating Simulator" << std::endl;
...
{
      std::cout << "Hello from simulator's constructor" << std::endl;

}

void Simulator::start()
{   
   std::cout << "Hello from simulator" << std::endl;
   // create a new Decoder object for this Simulator
   createDecoder();
   bool utopia = Sim()->getCfg()->getBool("perf_model/utopia/enabled");
   bool utopia_shadow_mode = Sim()->getCfg()->getBool("perf_model/utopia/shadow_mode_enabled");

   if (utopia || utopia_shadow_mode)
      m_utopia = new ParametricDramDirectoryMSI::Utopia();
   ...
   m_memory_allocator = new ParametricDramDirectoryMSI::BuddyAllocator(11); //TODO READ ORDER FROM CONFIG
   
   bool eager = Sim()->getCfg()->getBool("perf_model/rlb/enabled");
